'''
-sidepots is wrong somewhere, distribute less chips than exist, leaves negative amount in pot (maybe from Raise)

-display log of the actions that bots are taking

-end the game if only one player remains

-make leftPanelButtons inaccessible when out of order

-figure out why widgets (leftPanelButtons for one) are being resized and color changed

-change player1 avatar

-fold button sticks around
...
le "/Users/crazyfox/anaconda2/lib/python2.7/lib-tk/Tkinter.py", line 1541, in __call__
  ...
  File "currentPoker.py", line 181, in <lambda>
    self.b1 = tk.Button(self,text='Call '+str(min(room.table.plyrDct[plyr].stack_size,room.table.costToPlay-room.table.plyrDct[plyr].in_front)),highlightbackground='black',font=('Helvetica',16),command=lambda:self.call(plyr))
...
Poker---
 ...
 important vars == room.numberOfPlayers, room.stack_size, room.bigBlindSize
 room.table, room.table.playerOrder, room.table.plyrDct, room.table.leftToAct, room.table.inHand
 room.table.costToPlay, 
 room.imageList holds frames with visual player info, list is in same order as room.table.playerOrder
 dict room.imageList['player1'].c1 is the visual representation of room.plyrDct['player1'].hand[0], ie first card
...
class LeftPanelButtons(tk.Frame):
    ...
    def __init__(self,parent):
        ...
        self.deal = tk.Button(self.bg,takefocus=1, text='Deal Cards',bg='darkred',activebackground='darkred',highlightbackground='darkred',command=self.dealCards)
        self.deal.pack()
    ...
    def moveButton(self):
        ...
        for plyr in room.table.playerOrder[:]:
            if room.table.plyrDct[plyr].stack_size == 0:
    ...
    def showHands(self,event=None):
        for plyr in room.table.playerOrder:
            if room.table.plyrDct[plyr].hand != []:
                room.imageList[plyr].img1 = ImageTk.PhotoImage(Image.open('cardImages/'+room.table.plyrDct[plyr].hand[0]+'.gif'))
                room.imageList[plyr].c1.configure(image=room.imageList[plyr].img1)
                room.imageList[plyr].img2 = ImageTk.PhotoImage(Image.open('cardImages/'+room.table.plyrDct[plyr].hand[1]+'.gif'))
...
class PlayerWindow(tk.Frame):
    ...
    def createCheckButtons(self,plyr):
        ...
        self.wagerEntry = tk.Scale(self,from_=min(room.table.plyrDct[plyr].stack_size,room.table.minBet),to=room.table.plyrDct[plyr].stack_size,orient='horizontal',resolution=10,bg='black',fg='wheat3')
    ...
    def createCurrentPlayerImage(self,plyr):
        ...
        self.cardimg1 = ImageTk.PhotoImage(Image.open('cardImages/'+room.table.plyrDct[plyr].hand[0]+'.gif'))
        self.card1.configure(image=self.cardimg1)
        self.cardimg2 = ImageTk.PhotoImage(Image.open('cardImages/'+room.table.plyrDct[plyr].hand[1]+'.gif'))
...
# this populates the player area with current player info
    def populate(self):
        ...
        print(room.table.plyrDct, ' is plyrDct')
        print(room.table.leftToAct, ' is leftToAct')
        if room.table.leftToAct != []:
            if room.table.plyrDct[room.table.leftToAct[0]].stack_size == 0:
        ...
        # WORKING
        # IF PLAYER IS BOT
        if room.table.plyrDct[plyr].human == 0:
            self.plyrMsg.configure(text='Robot '+plyr+' is thinking...')
            self.createBotPlayerImage(plyr)
            # If pot is open
            if room.table.plyrDct[plyr].in_front == room.table.costToPlay:
                # do i need to pass the table object or can i refer to it from bot/player instance?
                botAction, maybeAmount = room.table.plyrDct[plyr].getRandomCheckAction(plyr,room.table)
                if botAction == 'check':
                    self.check(plyr)
                else:
                    self.bet(plyr,maybeAmount)
            # pot is not open
                ...
                botAction, maybeAmount = room.table.plyrDct[plyr].getRandomCallAction(plyr,room.table)
                if botAction == 'call':
                    self.call(plyr)
    ...
    def nextPlayer(self):
        ...
        if len(room.table.leftToAct) == 1 and len(room.table.inHand) == 1:
            tmp = room.table.pot
            room.table.plyrDct[room.table.leftToAct[0]].stack_size += room.table.pot
            room.table.pot = 0
            room.tableWindow.updateTableChips()
    ...
    def endRound(self):
        ...
        if room.table.leftToAct == [] and len(room.table.inHand) == 1: 
            tmp = room.table.pot
            room.table.plyrDct[room.table.inHand[0]].stack_size += room.table.pot
            room.table.pot = 0
            room.tableWindow.updateTableChips()
    ...
    def showWinners(self,pots,eligPlayers):
        ...
        if type(pots) == int: # no sidepots happened/sidepots would be type list
            ...
            self.cardimg1 = ImageTk.PhotoImage(Image.open('cardImages/'+room.table.plyrDct[plyrs[0]].hand[0]+'.gif'))
            self.card1.configure(image=self.cardimg1)
            self.cardimg2 = ImageTk.PhotoImage(Image.open('cardImages/'+room.table.plyrDct[plyrs[0]].hand[1]+'.gif'))
            self.card2.configure(image=self.cardimg2)
...
class TableWindow(tk.Frame):
    ...
    def updateTableChips(self):
        ...
        for plyr in room.table.playerOrder:
            room.imageList[plyr].stack.configure(text=room.table.plyrDct[plyr].stack_size)
    ...
    def createPlayerImages(self,plyrOrder):
        ...
        for plyr in plyrOrder:
            ...
            self.w.stack = tk.Label(self.w.info,text=room.table.plyrDct[plyr].stack_size,bg='black',fg='wheat3')
...
# gets initial values, ie room.numberOfPlayers, room.stack_size, room.bigBlindSize
class StartGameBar(tk.Frame):
    ...
    def seatPlayers(self):
        ...
        room.table.seatPlayers(plyrList,room.stack_size)
...
# stored as room.numberOfPlayers,room.stack_size,room.bigBlindSize
class StartGamePopup(object):
    def __init__(self,root):
        ...
        self.stack_size = tk.Label(self.top,bg='black',fg='burlywood1',text="What is the starting stack size?\nIncrements of 10, at least 100",font=('Courier bold',26))
        self.stack_size.pack()
        self.stack_sizeEntry = tk.Entry(self.top,relief='raised')
    ...
    def verify(self):
        ...
        try:
            ...
            if room.numberOfPlayers < 2 or room.numberOfPlayers > 9:
                raise
            room.stack_size = int(room.stack_size)
            if room.stack_size % 10 != 0 or room.stack_size < 100:
                raise
            room.bigBlindSize = int(room.bigBlindSize)
            if room.bigBlindSize % 20 != 0 or room.stack_size/10 < room.bigBlindSize:
                raise
            self.top.destroy()
        except:
            pass
